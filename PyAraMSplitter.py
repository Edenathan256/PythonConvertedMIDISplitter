...
limiter = 6400000 * 2 #limit to 6'400'000 events, each event is composed of 4 attributes.

#prepares slicing
n_spfalist = spfalist[1:]
n_spfaslice = []
...
for idx in range(iteration):
    ...
    if idx == 0:
        OutContent = MIDIName + "*" + SPFAsep #asterisk now indicates if it requires merging
    else:
        OutContent = SPFAsep
    OutContent = OutContent + (n_spfaslice[idx])
    OutMIDIName = "!" + MIDIName + "." + str(idx) + ".aramidi"
    #i hope this works
